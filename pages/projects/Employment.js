...
const Health = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      // Extract unique years with projects of type "Employment"
      const uniqueYears = [...new Set(response.filter(project => project.type === "Employment").map(project => project.projectYear))];
            ...
            <div className="card-header">
              <ul className="nav nav-tabs nav-tabs-neutral justify-content-start shadow p-3 mb-5 bg-body rounded">
                {years &&
                  years.map(
                    (year, index) =>
                      filterProjectsByTypeAndYear("Employment", year)
                        .length > 0 && (
